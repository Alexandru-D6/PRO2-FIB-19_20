...
int main() {
  ...
  while (not acabar) {    
    if (op == "escriure") {   
       ...
       if (not r.first) cout << endl << "L'estudiant amb DNI " << d <<
			  " no pertany al conjunt." << endl; 
       else cout << endl << "La posicio de l'estudiant amb DNI " << d <<
	      " es " << r.second << endl; 
    }
       ...
       int i;
       cin >> i; 
       cout << "La nota global de l'estudiant " << i << "-esim es " <<
	 c.consultar_iesim(i).consultar_nota_global() << endl;
    }
