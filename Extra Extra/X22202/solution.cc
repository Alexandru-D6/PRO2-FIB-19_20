     ...
     si b = fals, s'ha afegit l'estudiant est al par�metre impl�cit */
{
  ...
  if (not b)
    {
      if (est.te_nota()) {
        if (est.consultar_nota() >= 50.0) {
          ++nombre_aprovats;
          ++nombre_presentats;
        }else if (est.consultar_nota() < 50.0) ++nombre_presentats;
      }
     ...
     del par�metre impl�cit */
{
  ...
  if (b) {
    ...
    if (vest[i].te_nota()) {
      if (vest[i].consultar_nota() >= 50.0) {
        --nombre_aprovats;
        --nombre_presentats;
      }else if (vest[i].consultar_nota() < 50.0) --nombre_presentats;
    }
