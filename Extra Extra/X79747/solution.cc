     ...
     si b = fals, s'ha afegit l'estudiant est al par�metre impl�cit */
{
  ...
  if (not b) {
    if (est.te_nota()) {
      if (est.consultar_nota() < 5.00) {
        ++nombre_suspesos;
        if (est.consultar_nota() >= 4.00) ++nombre_compensables;
      }
    }else ++nombre_suspesos;
     ...
     del par�metre impl�cit */
{
  ...
  if (b) {
    ...
    // actualitzem la mida
    if (vest[i].te_nota()) {
      if (vest[i].consultar_nota() < 5.00) {
        --nombre_suspesos;
        if (vest[i].consultar_nota() >= 4.00) --nombre_compensables;
      }
    }else --nombre_suspesos;
