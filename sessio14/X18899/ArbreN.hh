...
#include "program.hh"
  ...
  static void arbsuma_rec(node_arbreNari* node, node_arbreNari* asuma, T& suma, const int N) {
    if (node == nullptr) {
      suma = 0;
      return;
    }else {
      suma += node->info;
      asuma->seg = vector<node_arbreNari*>(N);

      for (int i = 0; i < N; ++i) {
        arbsuma_rec(node->seg[i], asuma->seg[i], suma, N);
      }
    }
  }
  ...
  void arbsuma(ArbreNari& asum) const {
    T suma;
    node_arbreNari* aux = new node_arbreNari;
    arbsuma_rec(primer_node, aux, suma, N);
